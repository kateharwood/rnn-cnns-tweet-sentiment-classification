...
def train_model(model, loss_fn, optimizer, train_generator, dev_generator):
    ...
    # Make sure CNN does not run too long to stay under 10mins
    if type(model).__name__ == "ExperimentalNetwork2":
        max_epochs = 5
    else:
        max_epochs = 30
    ...
    for epoch in range(max_epochs): 
        for batch, labels in train_generator:
            optimizer.zero_grad()
            out_logits = model.forward(batch)
            loss = loss_fn(out_logits, labels)
            loss.backward()
            optimizer.step()

        print("\nEpoch ", epoch)
        print("Loss on validation data ")
        new_val_loss, f1_score = test_model(model, loss_fn, dev_generator)

        # Stop early if we've hit our F1 target and loss is not improving much
        if f1_score > 0.46 and new_val_loss >= best_val_loss:
            return model
...
# extension-grading
# Train the models with custom built learning rate schedulers
# Can choose "simple" or "step-based" scheduler
# "simple" updates the learning rate by gamma every step_size epochs
# "exponential-based" is based on the explanation in this paper: https://www.mdpi.com/2073-8994/12/4/660/pdf
def train_model_with_learning_rate_scheduler(model, loss_fn, optimizer, train_generator, dev_generator, gamma, learning_rate_method, step_size):
    best_val_loss = np.Infinity
    ...
    for epoch in range(30):
        ...
        # Update learning rate using a simple schedule method
        # extension-grading
        if (learning_rate_method == 'simple'):
            # Update learning rate every step_size epochs
            if epoch%step_size == 0 and epoch != 0:
                # Get current learning rate
                for param_group in optimizer.param_groups:
                    learning_rate = param_group['lr']
                new_learning_rate = learning_rate*gamma
                print("\nNew Learning Rate ", new_learning_rate)

                # Initialize optimizer with new learning rate
                # Normally it would be cleaner to use pytorch's built in learning rate scheduler here
                # But I am implementing a learning rate scheduler from scratch as a non-trivial extension
                optimizer = optim.Adam(model.parameters(), new_learning_rate)

        # Update the learning rate with a exponential-based schedule method
        # extension-grading
        if learning_rate_method == "exponential_based":
            # Implement the exponential-based learning schedule
            new_learning_rate = initial_learning_rate*(np.exp(-1*gamma*epoch))
            print("\nNew Learning Rate ", new_learning_rate)
            optimizer = optim.Adam(model.parameters(), new_learning_rate)
...
def test_model(model, loss_fn, test_generator):
    ...
    return loss, f1_score(gold, predicted, average='macro')
...
def main(args):
    ...
    # Get the network type to use
    if args.model == "dense":
        network = models.DenseNetwork(embeddings)
        learning_rate = 1e-3

    elif args.model == "RNN":
        network = models.RecurrentNetwork(embeddings)
        learning_rate = 2e-4
     
    # Extension 1: dense network using a learning rate scheduler for training
    # extension-grading
    elif args.model == "extension1":
        network = models.ExperimentalNetwork(embeddings)
        learning_rate = 2e-4
    
    # Extension 2: convolutional network
    # extension-grading
    elif args.model == "extension2":
        network = models.ExperimentalNetwork2(embeddings)
        learning_rate = 1e-2
    ...
    if args.model == "extension1":
        # Set the step_size for how often (after how many epochs) to update the learning rate
        step_size = 10
        # Set the gamma value for how much to decay the learning rate at each update
        gamma = 0.01
        # What type of custom scheduler to use
        scheduler_type = "exponential_based"
        network_model = train_model_with_learning_rate_scheduler(network, loss_fn, optimizer, train_generator, dev_generator, gamma, scheduler_type, step_size)
    else:
        network_model = train_model(network, loss_fn, optimizer, train_generator, dev_generator)

    # Test model
    print("\nTEST LOSS AND F1 SCORE")
    test_model(network_model, loss_fn, dev_generator)
